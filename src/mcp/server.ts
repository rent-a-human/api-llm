...
*/
  {
    ...
    description: "Generates a high-performance custom model from a series of CAD operations (sketch, extrude, revolve). Returns a local URL for the Jarvis Dashboard.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string", description: "Unique ID for the model" },
        name: { type: "string", description: "Descriptive name for the model" },
        operations: {
          type: "array",
          description: "Sequence of CAD operations: { type: 'extrude'|'revolve', sketch: [...], params: {...} }",
          items: { type: "object" }
        }
      },
...
export async function executeBackendTool(name: string, args: any): Promise<any> {
  ...
  switch (name) {
    case "local_cad_create_custom_model": {
      const { id, name: modelName, operations } = args as any;
      const result = await localCADClient.createCustomModel(id, modelName, operations);
      return {
        content: [
          {
            type: "text",
...
... (all cases from listing to change events)
    }
*/
